...
void insert(CRECID *pq, CRECID new_example, int size) {
    int j = size - 1, p;
    pq[j] = new_example;
    while ( (p=(j-1)/2) >= 0 ) {
...
void q_delete(CRECID *pq, int size) {
    int j, p = 0;
    pq[p] = pq[size - 1];
...
int merge_write(CRECID new_example, CRECID *old, FILE *fout) {
    if (new_example.word1 == old->word1 && new_example.word2 == old->word2) {
        old->val += new_example.val;
...
int merge_files(int num) {
    ...
    for (i = 0; i < num; i++) {
        ...
        fread(&new_example, sizeof(CREC), 1, fid[i]);
        new_example.id = i;
        insert(pq,new_example,i+1);
    }
